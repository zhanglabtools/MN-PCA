...
def W2(data,truedata,LAMBDA_A = 0.01,LAMBDA_B = 0.01,sigma = 0.1,PROTECT_NUMBER = 1, n_iter=400):           
    ...
    UPDATE_ITERS_B = 5
    TRANS_ITERS = n_iter
    statistic = np.zeros(TRANS_ITERS*3)
    statistic.shape = (TRANS_ITERS,3) 
    ...
    zero[torch.arange(PROTECT_NUMBER).long()]=1
    protect = zero
    unprotect = one-zero
    if use_cuda:
    ...
    for iters in range(TRANS_ITERS):
        ...
        principle = principle.float()
        principle = torch.mm(torch.inverse(G.line1.weight),torch.mm(principle,torch.inverse(T.line1.weight.t())))
        if use_cuda:
        ...
        for epoch in range(epochs):    
            for iter_g in range(UPDATE_ITERS_A):
                ...
                try:
                    ...
                    if fake.shape[0]>fake.shape[1]:
                        zero = torch.zeros(fake.shape[0]-fake.shape[1],fake.shape[1])
                        if use_cuda:
                            zero = zero.cuda()
                        ident = torch.cat((ident,zero),0)
                        fake = fake + 0.001*ident
                    if fake.shape[0]==fake.shape[1]:
                        fake = fake + 0.001*ident
                    if fake.shape[0]<fake.shape[1]:
                        zero = torch.zeros(fake.shape[0],fake.shape[1]-fake.shape[0])
                        if use_cuda:
                            zero = zero.cuda()
                        ident = torch.cat((ident,zero),1)
                        fake = fake + 0.001*ident
                    u,s,v = torch.svd(fake)
                s = torch.abs(s)
                Wasserstein_Distance1 = 1.0/math.sqrt((number_col-1)*(number_row-1))*torch.trace(covariance)-2.0*sigma*math.sqrt(math.sqrt(1.0/((number_col-1)*(number_row-1))))*torch.sum(s)
                ...
                alphaA = torch.mean(torch.diag(A))
            ...
            for iter_g in range(UPDATE_ITERS_B):
                ...
                try:
                    ...
                    u,s,v = torch.svd(fake)
                s = torch.abs(s)
                Wasserstein_Distance2 = 1.0/math.sqrt((number_col-1)*(number_row-1))*torch.trace(covariance)-2.0*sigma*math.sqrt(math.sqrt(1.0/((number_col-1)*(number_row-1))))*torch.sum(s)
                ...
                alphaB = torch.mean(torch.diag(B))
                ...
                optimizerG.step()
            alpha = math.sqrt(alphaB/alphaA)
            #T.line1.weight.data *=1/alphaA
            T.line1.weight.data *=(alphaA+alphaB)/(2*alphaA)
            #G.line1.weight.data *=1/alpha
            #print(A[abs(A)>0.01].shape)
            #print(B[abs(B)>0.01].shape)                     
        ...
        if iters % 20 == 0:
            print("Iter = %d %.4f %.4f %.4f" % (iters, statistic[iters, 0], 
                statistic[iters, 1], statistic[iters, 2]))
