function res_dir = run_experiment_w2r(X, label, dim, output_dir, opts)
...
rng('default')
rng(0);
[X_, w2_iA, w2_iB] = MnPCAq1_wrapper(X, dim, opts.lam1, opts.lam2, opts.sig, randn(size(X)), n_iter); %#ok<ASGLU>
w2_score = pca(X_, dim);
%% evaluation
w2_mvres = gen_mvres();
